  ...
  try {
    ...
    if (ANTHROPIC_API_KEY) {
      const claudeRes = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 800, messages: [{ role: 'user', content: `Format these song lyrics for text-to-speech singing. Remove structural tags like [Verse], [Chorus], [Bridge] but keep all original words exactly. Add natural pauses using commas and periods for singing rhythm. Return ONLY the formatted lyrics.\n\nLyrics:\n${lyrics}` }] })
      });
      if (claudeRes.ok) {
        const claudeData = await claudeRes.json();
        const formatted = claudeData.content?.[0]?.text?.trim();
        if (formatted) formattedLyrics = formatted;
      }
    }
    ...
    const ttsRes = await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${voice_id}`, {
      ...
      body: JSON.stringify({ text: formattedLyrics, model_id: 'eleven_multilingual_v2', voice_settings: { stability: 0.4, similarity_boost: 0.85, style: 0.5, use_speaker_boost: true } }),
    });

    if (!ttsRes.ok) {
      const err = await ttsRes.text();
      const { data: p } = await supabase.from('profiles').select('available_credits').eq('user_id', user.id).single();
      if (p) { await supabase.from('profiles').update({ available_credits: p.available_credits + CREDITS_COST }).eq('user_id', user.id); await supabase.from('credit_transactions').insert({ user_id: user.id, amount: CREDITS_COST, type: 'refund', description: 'Reembolso: fallo vocal' }); }
    ...
    const { data: generation } = await supabase.from('ai_generations').insert({ user_id: user.id, prompt: `Pista vocal: ${voice_name || 'Voz clonada'} | ${genre || ''} ${mood || ''}`.trim(), genre: genre || null, mood: mood || null, audio_url: audioUrl, duration: 0 }).select().single();
