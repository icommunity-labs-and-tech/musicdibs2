            ...
            <Card key={module.titleKey} className={`relative overflow-hidden transition-all duration-300 ${!module.available ? 'opacity-50 grayscale pointer-events-none' : disabled ? 'opacity-60 grayscale' : 'hover:shadow-lg hover:-translate-y-1'}`}>
              ...
              <CardHeader>
                ...
                {module.costsCredits && cost > 0 ? (
                  ...
                  <span className="mt-1 inline-flex items-center rounded-full bg-emerald-500/10 border border-emerald-500/30 px-2.5 py-0.5 text-[11px] font-semibold text-emerald-500">{t('aiStudio.free', 'Gratis')}</span>
                ) : null}
              </CardHeader>
              <CardContent>
                {!module.available ? (
