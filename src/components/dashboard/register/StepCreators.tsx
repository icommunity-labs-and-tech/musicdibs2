  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Añade los autores y colaboradores de la obra.</p>
      </div>
          ...
          <Card key={c.id} className="border-border/40">
            <CardContent className="p-4 space-y-3">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-muted-foreground">Creador {idx + 1}</p>
                {creators.length > 1 && (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="space-y-1">
                  ...
                  <Input value={c.name} onChange={(e) => update(c.id, { name: e.target.value })} placeholder="Nombre del creador" className="h-9" />
                </div>
                <div className="space-y-1">
              ...
              <div className="space-y-1">
                <Label className="text-xs">% de derechos <span className="text-muted-foreground">(opcional)</span></Label>
                <Input
                  type="number"
                  min={0}
                  max={100}
      ...
      {usesPercentages && totalPct !== 100 && (
        <p className="text-xs text-destructive">Los porcentajes deben sumar 100%.</p>
      )}
